...
| [design-decisions.md](./design-decisions.md) | DD-1 through **DD-16**: rationale for stack choices, Headless vs Atomic, client-side ML wiring (ART), client-side BST tier ranges (vs IPE), separate seeder package, detail-page Coveo bypass, CSS3-only scraping selectors, **HTTP security headers + CSP + `next/image`**, analytics-mode `'legacy'` pinning, and **detail-page catalog UI** (stacked cards, light sheet, stat bar fills). |
