...
### 3.4. Query Suggestions (QS) model — **Shipped (Advanced challenge item)**
...
### 3.6. Relevance Generative Answering (RGA) — **Shipped (Advanced challenge item)**
...
### 3.9. Passage Retrieval API — **Bonus challenge item**
...
- **Brief explicitly accepts a written POV** in lieu of a build — minimum bonus deliverable is documenting *how* you would use it.
...
| ~~4~~ | ~~Query Suggestions (Advanced)~~ — **Shipped (model status: Limited until training data accumulates)** | — | — |
| 5 | Search-within-facet on Ability | ~1 hr app change | App only |
| ~~6~~ | ~~RGA (Advanced, if quota allows)~~ — **Shipped (model: Active)** | — | — |
| ~~7~~ | ~~ART~~ — **Shipped (model + wiring); model status: Build in progress** | — | — |
| 8 | Ranking expressions | Admin only | Admin only |
| 9 | Passage Retrieval POV (Bonus) | Doc only | Docs |
| 10 | DNE / Facet Generator | Larger refactor | Deferred |

Items 1–4 are the near-mandatory polish for the submission. Items 5–6 are where depth shows. Items 7–9 are the "talk like a Coveo SE" layer for the presentation.
