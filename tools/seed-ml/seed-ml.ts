 ...
 *
 * Result-card clicks are intentionally NOT exercised: the app today renders
 * `<a target="_blank">` for results without a `buildInteractiveResult` wrapper,
 * so a click emits no `documentOpen` analytics — clicking would be theatre, not
 * training signal. Fixing that is a separate follow-up.
 *
...
if (!["all", "qs", "facets", "rga"].includes(BUCKET)) {
  console.error(`[seeder] --bucket must be one of all|qs|facets|rga (got: ${BUCKET})`);
  process.exit(1);
}
...
async function runQuerySuggestionSeed(page: Page, query: string): Promise<void> {
  ...
  if (await isVisibleSoon(firstSuggestion, 250)) {
...
function pickQueriesForBucket(): {
  ...
  switch (BUCKET) {
    case "qs":
      return { qs: config.querySuggestionSeeds, facets: [], rga: [] };
    case "facets":
      return { qs: [], facets: config.facetExerciseSeeds, rga: [] };
    case "rga":
      return { qs: [], facets: [], rga: config.naturalLanguageRGASeeds };
    case "all":
    default:
      return {
...
async function main(): Promise<void> {
  ...
  const totalActions =
    (buckets.qs.length + buckets.facets.length + buckets.rga.length) * LOOPS;
  let actionIndex = 0;
